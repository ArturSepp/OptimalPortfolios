...
- Added optional `fixed_weight_assets` to inverse risk-budget inference. Named
  ...
  full-covariance fit and receive zero reported budget, independently of the
  automatic non-positive-contribution rule.
- A failed inverse fit now probes free overweight assets at their minimum
...
- Central-weight assets with non-positive average risk contributions are
...
- Removed the unreleased legacy residual-covariance object, estimator,
  ...
  Prepared residual correlation is the only empirical state; rebuild old
  development snapshots from source data. No compatibility shims are retained.
...
- Revised the opt-in empirical residual risk model to preserve current MATF
  ...
  default and native annual alpha are unchanged; no factor–residual cross term.
...
- `FactorCovarEstimator` and `estimate_lasso_factor_covar_data` accept
  ...
  Native frequency/span/unit metadata prepare a full annual residual covariance
  in FactorLasso on complete common log-return periods. Monthly/quarterly inputs
  default to QE; no span or scale is required at covariance retrieval.
