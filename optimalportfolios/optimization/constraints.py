...
class GroupLowerUpperConstraints:
    ...
    Attributes:
        group_loadings: Binary matrix (groups x instruments) where 1 indicates membership
        group_min_allocation: Minimum allocation per group
        group_max_allocation: Maximum allocation per group
    """
...
class GroupTrackingErrorConstraint:
    ...
    Attributes:
        group_loadings: Binary matrix (groups x instruments)
...
class GroupTurnoverConstraint:
    ...
    Attributes:
        group_loadings: Binary matrix (groups x instruments)
        group_max_turnover: Maximum turnover per group
    """
    group_loadings: pd.DataFrame
...
class Constraints:
    ...
    Attributes:
        is_long_only: Enforce non-negative weights
        min_weights: Minimum weight per asset
        max_weights: Maximum weight per asset
        max_exposure: Maximum total exposure
        min_exposure: Minimum total exposure
        benchmark_weights: Benchmark weights for tracking error
        tracking_err_vol_constraint: Maximum tracking error volatility
        weights_0: Current weights for turnover calculations
        turnover_constraint: Maximum portfolio turnover
        turnover_costs: Transaction costs per asset
        target_return: Target portfolio return
        asset_returns: Expected asset returns
        max_target_portfolio_vol_an: Maximum portfolio volatility
        min_target_portfolio_vol_an: Minimum portfolio volatility
        group_lower_upper_constraints: Group allocation constraints
        group_tracking_error_constraint: Group tracking error constraints
        group_turnover_constraint: Group turnover constraints
    """
    ...
    def set_cvx_utility_objective_constraints(
            ...
            covar: Union[np.ndarray, psd_wrap] = None,
            exposure_scaler: cvx.Variable = None,
            tre_utility_weight: Optional[float] = None,
        turnover_utility_weight: Optional[float] = None
        ...
        tracking_error_var = cvx.quad_form(w - benchmark_weights.to_numpy(), covar)
        if alphas is not None: # alphas are added
            objective_fun = alphas.T @ (w - benchmark_weights.to_numpy())
        else:
            objective_fun = []

        if turnover_utility_weight is not None:
            # Group turnover constraints
            if self.group_turnover_constraint is not None:
                if self.weights_0 is None:
                    ...
                    for group in self.group_turnover_constraint.group_loadings.columns:
                        ...
                        if np.any(np.isclose(group_loading, 0.0) == False):
                            ...
                            objective_fun += -1.0*turnover_utility_weight * (
                                cvx.norm( cvx.multiply(group_loading.to_numpy(), w - self.weights_0), 1)
            ...
            # Portfolio-level turnover constraint
                ...
                if self.weights_0 is None:
                    ...
                    if self.turnover_costs is not None:
                        objective_fun += -1.0*turnover_utility_weight * (
                                cvx.norm(cvx.multiply(self.turnover_costs.to_numpy(), w - self.weights_0), 1)
                                - self.turnover_constraint)
                    else:
                        assert w.size == len(self.weights_0.index)
                        objective_fun += -1.0*turnover_utility_weight * (
                                cvx.norm(w - self.weights_0, 1)
                                - self.turnover_constraint)

        # Group tracking error constraints
        if tre_utility_weight is not None:
            if self.group_tracking_error_constraint is not None:
                if self.benchmark_weights is None:
                    ...
                    for group in self.group_tracking_error_constraint.group_loadings.columns:
                        ...
                        if np.any(np.isclose(group_loading, 0.0) == False):
                            group_loading = group_loading.loc[benchmark_weights.index]
                            objective_fun += -1.0*tre_utility_weight * (cvx.quad_form(
                                cvx.multiply(group_loading.to_numpy(), w - benchmark_weights.to_numpy()), covar)
                                -self.group_tracking_error_constraint.group_tre_vols.loc[group] ** 2)

            # Portfolio-level tracking error constraint
            else:
                if benchmark_weights is None:
                    raise ValueError("benchmark_weights must be given for tracking error constraint")
                objective_fun += -1.0*tre_utility_weight * ( cvx.quad_form(w - benchmark_weights.to_numpy(), covar)
                                                             - self.tracking_err_vol_constraint ** 2)
