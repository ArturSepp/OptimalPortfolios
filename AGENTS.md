...
uv run pytest                                            # run the test suite (1310 tests, ~3 min)
...
Line coverage measured **99.80%** on the 1310-test dev suite. The
ubuntu/3.12 matrix entry gates `pytest --cov=optimalportfolios` at `fail_under = 99.7`; this floor rises
...
`optimalportfolios[data]`; neither enabled a single test — collection is 1310 either way. The
