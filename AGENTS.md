...
pytest                                                   # run the test suite (1072 tests, ~30 s)
...
Full-package line coverage measured **89.30%** on the 1072-test dev suite. The Python 3.12
...
- **Ruff is configured in `ruff.toml`, not in `pyproject.toml`.** Ruff reads `ruff.toml` in preference, so the two must never both define the config.
...
- `ruff check optimalportfolios/` reports 431 baseline findings: 379 `E501` line-length, and the small `E702`/`E712`/`E402`/`W291`/`W292` remainder. CI gates TID251/TID253/ICN **and `F`**, all green; `E`/`W` remain ungated by policy. Fix only the lines your specific change touches; a repository-wide reflow is not wanted.
- **`F401` in an `__init__.py` is a re-export, not an unused import.** Every subpackage declares an `__all__` stating its export surface; that is what makes those imports "used", and what makes the star imports in the top-level `__init__.py` auditable. Adding a name to a subpackage's public API means adding it to that `__all__` in the same file. Never `ruff --fix` F401 across `__init__.py` without reading what it would delete — it would break `from optimalportfolios import Constraints` for every consumer.
