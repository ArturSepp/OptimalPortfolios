...
uv run pytest                                            # run the test suite (1276 tests, ~3 min)
...
It has two lanes, and the split is **derived rather than listed**: `.github/scripts/run_examples.py` walks each example's intra-`examples` import closure and calls it network-bound if `yfinance` is reachable at all. Of 26 examples, 21 are — 8 import it directly and the rest reach it through `examples/data/universe.py`, whose `fetch_benchmark_universe_data()` downloads 15 tickers back to 2003, while `sp500_universe.py` pulls the whole index. The **offline** lane runs the other 5 on all three runners and gates pull requests; it syncs the *core* environment, so it shows the examples work for someone who ran a plain `pip install optimalportfolios`, and it finishes in about ten seconds. The **network** lane runs the 21 on a daily schedule only, `continue-on-error`, with a step-summary report — gating a PR on 63 live Yahoo downloads would fail on Yahoo's availability far more often than on the diff, and a check that is usually red for unrelated reasons is one people learn to ignore. The script fails on an empty lane, so a classification bug cannot report success by running nothing. Add an example and it is classified automatically; a hand-kept list of five paths would be wrong the first time someone did.
...
`optimalportfolios[data]`; neither enabled a single test — collection is 1276 either way. The
