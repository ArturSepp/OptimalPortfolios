...
The test dependencies live in the PEP 735 `test` dependency group. The pinned CI cell uses
`uv sync --locked --group test`; floating matrix cells use `uv sync --group test --upgrade` to
exercise current compatible releases. The lint
tools are deliberately **not** in the `test` group: they are declared once in the `lint`
